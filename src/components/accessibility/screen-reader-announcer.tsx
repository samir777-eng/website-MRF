...
export function ScreenReaderAnnouncerProvider({
  ...
  const announceQuestion = useCallback(
    (questionNumber: number, totalQuestions: number, questionText: string) => {
      announce(
        `السؤال ${questionNumber} من ${totalQuestions}: ${questionText}`,
        "assertive",
      );
    },
    [announce],
  ...
  const announceQuizStart = useCallback(
    (quizTitle: string, questionCount: number, duration: number) => {
      announce(
        `بدء الاختبار: ${quizTitle}. ${questionCount} سؤال في ${duration} دقيقة.`,
        "assertive",
      );
    },
    [announce],
  ...
  const announceQuizEnd = useCallback(
  ...
  const announceAchievement = useCallback(
    (achievementName: string, xpGained?: number) => {
  ...
  const announceXPGain = useCallback(
    (xpAmount: number, reason?: string) => {
  ...
  const announceStreakUpdate = useCallback(
    (streakDays: number) => {
      if (streakDays === 1) {
