...
const preQuizData = {
  ...
  questions: [
    { id: 1, question: "ما هو تعريف النحو؟", options: ["علم يبحث في أصول الكلام", "علم يبحث في أواخر الكلمات", "علم يبحث في معاني الكلمات", "علم يبحث في أصوات الحروف"], correct: 1 },
    { id: 2, question: "ما هي أقسام الكلام في اللغة العربية؟", options: ["اسم وفعل", "اسم وفعل وحرف", "اسم وحرف", "فعل وحرف"], correct: 1 },
    { id: 3, question: "ما هو الإعراب؟", options: ["تغيير أواخر الكلمات", "تغيير أوائل الكلمات", "تغيير أوساط الكلمات", "لا شيء مما سبق"], correct: 0 },
    { id: 4, question: "ما هو المبتدأ؟", options: ["اسم مرفوع يقع في أول الجملة الاسمية", "اسم منصوب", "اسم مجرور", "فعل ماضٍ"], correct: 0 },
    { id: 5, question: "ما هو الخبر؟", options: ["ما يتم به معنى المبتدأ", "ما يسبق المبتدأ", "الفعل الماضي", "الحرف"], correct: 0 },
  ],
};
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <Card className="max-w-2xl mx-auto border-0 shadow-xl">
            <CardContent className="p-8 text-center space-y-6">
              ...
              <p className="text-muted-foreground">{preQuizData.lectureTitle}</p>
              ...
              <div className="p-4 bg-amber-100 dark:bg-amber-900/20 rounded-lg">
                <div className="flex items-center gap-2 justify-center text-amber-800 dark:text-amber-200">
                  ...
                  <span>يجب الحصول على {preQuizData.threshold}% على الأقل لفتح المحاضرة</span>
                </div>
              </div>

              <div className="grid grid-cols-3 gap-4 py-4">
                <div className="text-center">
                  <div className="text-2xl font-bold">{preQuizData.questions.length}</div>
                  <div className="text-sm text-muted-foreground">سؤال</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">{Math.floor(preQuizData.duration / 60)}</div>
                  <div className="text-sm text-muted-foreground">دقيقة</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">{attemptsLeft}</div>
                  <div className="text-sm text-muted-foreground">محاولات متبقية</div>
                </div>
              </div>

              {attemptsLeft > 0 ? (
                <Button size="lg" onClick={startQuiz} className="w-full gap-2 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600">
                  <Play className="h-5 w-5" />
                  بدء الاختبار
                </Button>
              ) : (
                <div className="p-4 bg-red-100 dark:bg-red-900/20 rounded-lg">
                  <p className="text-red-800 dark:text-red-200 font-medium">لقد استنفدت جميع المحاولات. يرجى التواصل مع الدعم.</p>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="p-8 text-center space-y-6">
              ...
              {passed ? (
                ...
                <div className="space-y-4">
                  ...
                  {preQuizData.maxAttempts - preQuizData.currentAttempts - 1 > 0 ? (
                    <Button size="lg" onClick={() => setState("intro")} className="w-full gap-2" variant="outline">
        ...
        <Card className="max-w-3xl mx-auto border-0 shadow-xl">
          <CardContent className="p-8 space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              <Button variant="outline" onClick={prevQuestion} disabled={currentQuestion === 0} className="flex-1">السابق</Button>
              <Button onClick={nextQuestion} disabled={selectedAnswer === null} className="flex-1">{currentQuestion === preQuizData.questions.length - 1 ? "إنهاء" : "التالي"}</Button>
            </div>
          </CardContent>
        </Card>
      </div>
