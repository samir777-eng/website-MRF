...
test.describe('Responsive UI Tests - All Viewports', () => {
  const viewports = [
    ...
    { name: 'tablet-large', width: 1024, height: 1366, description: 'iPad Pro' },
    { name: 'desktop', width: 1280, height: 720, description: 'Laptop' },
    { name: 'desktop-large', width: 1920, height: 1080, description: 'Full HD' },
    { name: 'desktop-xl', width: 2560, height: 1440, description: '2K Monitor' },
  ];
    ...
    test.describe(`${viewport.name} (${viewport.width}x${viewport.height})`, () => {
      ...
      test('navigation accessible', async ({ page }) => {
        ...
        if (viewport.width < 768) {
          ...
          const menuButton = page.locator('button[aria-label*="menu"], button[aria-label*="قائمة"]').first();
