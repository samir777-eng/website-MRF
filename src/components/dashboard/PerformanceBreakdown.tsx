        ...
        <div className="bg-background/95 backdrop-blur-sm border border-border rounded-lg p-4 shadow-xl">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <span
                className={`font-bold ${data.score >= data.target ? "text-green-600" : "text-red-600"}`}
              >
                {data.score >= data.target ? "+" : ""}
                {data.score - data.target}%
              </span>
            </div>
            <Badge className={statusLabel.color}>{statusLabel.text}</Badge>
          </div>
        </div>
      );
    }
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          ...
          <div className="bg-blue-100 dark:bg-blue-900/20 rounded-lg p-3">
            <div className="text-2xl font-bold text-blue-700 dark:text-blue-400">
              {data.length - excellentCount - needsImprovementCount}
            </div>
              ...
              >
                {data.map((entry, index) => (
                  <Cell
                    ...
                    opacity={
                      selectedBar === null || selectedBar === entry.subject
                        ? 1
                        : 0.3
                    }
                  />
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-orange-800 dark:text-orange-200">
                  ركز على المواد التي تحتاج تحسين:{" "}
                  {data
                    .filter((d) => d.score < 70)
                    .map((d) => d.subject)
                    .join("، ")}
