...
interface SpacedRepetitionContextType {
  ...
  startReviewSession: (sessionType: ReviewSession['sessionType'], maxItems?: number) => ReviewSession | null;
...
export function SpacedRepetitionProvider({ children }: SpacedRepetitionProviderProps) {
  ...
  const itemsDueToday = getItemsDueForReview(reviewItems, new Date(), 50).filter(item => {
    const today = new Date();
    const itemDate = new Date(item.nextReviewDate);
    return itemDate.toDateString() === today.toDateString();
  });
  const itemsOverdue = getItemsDueForReview(reviewItems, new Date(), 50).filter(item => {
    const today = new Date();
  ...
  const addReviewItem = useCallback((
    ...
    const newItem = createReviewItem(contentId, contentType, title, description, difficulty, subject, tags);
    setReviewItems(prev => [...prev, newItem]);
  }, []);

  // Update review item
  const updateReviewItemData = useCallback((itemId: string, updates: Partial<ReviewItem>) => {
  ...
  const submitReviewResponse = useCallback((response: ReviewResponse) => {
    ...
    setReviewItems(prev => prev.map(i => i.id === response.itemId ? updatedItem : i));
    ...
    setCurrentSession(prev => {
      ...
      const newAvgResponseTime = 
        (prev.averageResponseTime * prev.itemsReviewed.length + response.responseTime) / 
  ...
  const endReviewSession = useCallback(() => {
    ...
    const sessionAccuracy = currentSession.correctAnswers / Math.max(currentSession.itemsReviewed.length, 1);
  ...
  const getItemsDue = useCallback((maxItems: number = 20) => {
    return getItemsDueForReview(reviewItems, new Date(), maxItems);
  }, [reviewItems]);

  // Check if review is due
  const isReviewDue = useCallback((itemId: string) => {
