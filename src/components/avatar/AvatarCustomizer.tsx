...
export function AvatarCustomizer({
  ...
  const [selectedCategory, setSelectedCategory] = useState<AvatarPartType>("skin");
  const [previewConfig, setPreviewConfig] = useState<Partial<AvatarConfig> | null>(null);
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-800">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-display text-xl font-bold">تخصيص الصورة الشخصية</h2>
              </div>
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex gap-1 p-3 overflow-x-auto border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/30 scrollbar-hide">
                  {CATEGORY_ORDER.map((type) => {
                    ...
                    const ownedCount = categoryParts.filter((p) => ownedParts.includes(p.id)).length;
                    ...
                    return (
                      ...
                      >
                        <div className={`w-6 h-6 ${isActive ? "text-white" : "text-gray-500 dark:text-gray-400"}`}>
                          {CATEGORY_ICONS[type]}
                        </div>
                        <span className="text-[10px]">{PART_TYPE_LABELS[type]}</span>
                        <span className={`text-[9px] ${isActive ? "text-purple-200" : "text-gray-400"}`}>
                      ...
                      return (
                        ...
                        >
                          <div className="flex flex-col items-center gap-2">
                            <PartPreview part={part} selectedCategory={selectedCategory} />
                        ...
                        <div className="flex items-center justify-between gap-4">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {PART_TYPE_LABELS[previewingPart.type]} • {getRarityLabel(previewingPart.rarity)}
                            </p>
                          </div>
...
function PartPreview({ part, selectedCategory }: { part: AvatarPart; selectedCategory: AvatarPartType }) {
  // Color swatches for skin, hair color, and background
  if ((selectedCategory === "skin" || selectedCategory === "hairColor" || selectedCategory === "background") && part.value) {
    const isGradient = part.value.includes("gradient") || part.value.includes("linear");
  ...
  return (
    <div className="w-12 h-12 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden">
      <svg viewBox="0 0 48 48" width={40} height={40}>
        ...
        {selectedCategory === "eyebrows" && <EyebrowsPreview browsId={part.id} />}
        {selectedCategory === "mouth" && <MouthPreview mouthId={part.id} />}
        {selectedCategory === "glasses" && <GlassesPreview glassesId={part.id} />}
        {selectedCategory === "accessory" && <AccessoryPreview accessoryId={part.id} />}
        {selectedCategory === "clothing" && <ClothingPreview clothingId={part.id} />}
      </svg>
    </div>
  );
}
...
function parseGradient(value: string): string {
  if (value.includes("sunset")) return "linear-gradient(135deg, #F97316 0%, #EC4899 100%)";
  if (value.includes("ocean")) return "linear-gradient(135deg, #06B6D4 0%, #3B82F6 100%)";
  if (value.includes("forest")) return "linear-gradient(135deg, #10B981 0%, #059669 100%)";
  if (value.includes("galaxy")) return "linear-gradient(135deg, #8B5CF6 0%, #EC4899 50%, #F97316 100%)";
  if (value.includes("gold")) return "linear-gradient(135deg, #F59E0B 0%, #D97706 100%)";
  return value;
}
...
function FacePreview({ faceId }: { faceId: string }) {
  ...
  switch (faceId) {
    ...
    case "face-heart":
      return <path d="M24 8 C14 8 8 18 8 26 C8 36 16 42 24 46 C32 42 40 36 40 26 C40 18 34 8 24 8" fill={color} />;
    default:
...
function HairPreview({ hairId }: { hairId: string }) {
  ...
  switch (hairId) {
    case "hair-none":
      return <circle cx="24" cy="24" r="12" fill="#E5E7EB" stroke="#D1D5DB" strokeWidth="1" strokeDasharray="2 2" />;
    case "hair-short":
      return <path d="M10 22 Q10 8 24 6 Q38 8 38 22 Q36 12 24 10 Q12 12 10 22" fill={color} />;
    case "hair-medium":
      return <path d="M8 26 Q8 6 24 4 Q40 6 40 26 Q38 12 24 10 Q10 12 8 26" fill={color} />;
    case "hair-long":
      return (
        <>
          <path d="M8 26 Q8 6 24 4 Q40 6 40 26 L42 44 Q24 46 6 44 Z" fill={color} />
          <path d="M8 26 Q8 6 24 4 Q40 6 40 26 Q38 12 24 10 Q10 12 8 26" fill={color} />
        </>
      );
    case "hair-curly":
      return (
    ...
    case "hair-wavy":
      return <path d="M8 22 Q6 12 14 8 Q20 4 24 6 Q28 4 34 8 Q42 12 40 22 Q38 14 24 12 Q10 14 8 22" fill={color} />;
    case "hair-spiky":
      return (
        <>
    ...
    case "hair-ponytail":
      return (
        <>
          <path d="M10 22 Q10 8 24 6 Q38 8 38 22 Q36 12 24 10 Q12 12 10 22" fill={color} />
...
function EyesPreview({ eyesId }: { eyesId: string }) {
  switch (eyesId) {
    case "eyes-happy":
      return (
        <>
          <path d="M14 24 Q18 20 22 24" stroke="#4A3728" strokeWidth="2" fill="none" strokeLinecap="round" />
          <path d="M26 24 Q30 20 34 24" stroke="#4A3728" strokeWidth="2" fill="none" strokeLinecap="round" />
        </>
      );
    case "eyes-sleepy":
      return (
        <>
          <ellipse cx="18" cy="24" rx="4" ry="2" fill="white" stroke="#4A3728" strokeWidth="1" />
          <ellipse cx="30" cy="24" rx="4" ry="2" fill="white" stroke="#4A3728" strokeWidth="1" />
          <line x1="14" y1="22" x2="22" y2="23" stroke="#4A3728" strokeWidth="1.5" />
          <line x1="26" y1="23" x2="34" y2="22" stroke="#4A3728" strokeWidth="1.5" />
        </>
      );
    case "eyes-wink":
      return (
        <>
          <ellipse cx="18" cy="24" rx="4" ry="3" fill="white" stroke="#4A3728" strokeWidth="1" />
    ...
    case "eyes-stars":
      return (
        <>
          <polygon points="18,20 19,23 22,23 20,25 21,28 18,26 15,28 16,25 14,23 17,23" fill="#FFD700" />
          <polygon points="30,20 31,23 34,23 32,25 33,28 30,26 27,28 28,25 26,23 29,23" fill="#FFD700" />
        </>
      );
    case "eyes-heart":
      return (
        <>
          <path d="M18 27 C14 23 14 20 16 19 C18 18 19 20 18 22 C17 20 18 18 20 19 C22 20 22 23 18 27" fill="#EC4899" transform="scale(0.8) translate(4, 4)" />
          <path d="M30 27 C26 23 26 20 28 19 C30 18 31 20 30 22 C29 20 30 18 32 19 C34 20 34 23 30 27" fill="#EC4899" transform="scale(0.8) translate(4, 4)" />
        </>
      );
    default:
      return (
        <>
          <ellipse cx="18" cy="24" rx="4" ry="3" fill="white" stroke="#4A3728" strokeWidth="1" />
          <circle cx="18" cy="24" r="2" fill="#4A3728" />
          <ellipse cx="30" cy="24" rx="4" ry="3" fill="white" stroke="#4A3728" strokeWidth="1" />
...
function EyebrowsPreview({ browsId }: { browsId: string }) {
  ...
  switch (browsId) {
    case "brows-raised":
      return (
        <>
          <path d="M12 18 Q18 14 24 18" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />
          <path d="M24 18 Q30 14 36 18" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />
        </>
      );
    case "brows-angry":
      return (
        <>
          <path d="M12 22 Q18 18 24 20" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />
          <path d="M24 20 Q30 18 36 22" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />
        </>
      );
    case "brows-worried":
      return (
        <>
          <path d="M12 19 Q18 22 24 19" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />
          <path d="M24 19 Q30 22 36 19" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />
        </>
      );
    case "brows-unibrow":
      return <path d="M10 20 Q24 14 38 20" stroke={color} strokeWidth="2.5" fill="none" strokeLinecap="round" />;
    default:
      return (
        <>
          <path d="M12 20 Q18 18 24 20" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />
          <path d="M24 20 Q30 18 36 20" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />
        </>
      );
  }
}
...
function MouthPreview({ mouthId }: { mouthId: string }) {
  ...
  switch (mouthId) {
    case "mouth-grin":
      return <path d="M14 28 Q24 38 34 28" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />;
    case "mouth-neutral":
      return <line x1="18" y1="30" x2="30" y2="30" stroke={color} strokeWidth="2" strokeLinecap="round" />;
    case "mouth-open":
    ...
    default:
      return <path d="M16 28 Q24 36 32 28" stroke={color} strokeWidth="2" fill="none" strokeLinecap="round" />;
  }
}
...
function GlassesPreview({ glassesId }: { glassesId: string }) {
  ...
  switch (glassesId) {
    case "glasses-none":
      return <circle cx="24" cy="24" r="10" fill="none" stroke="#D1D5DB" strokeWidth="1" strokeDasharray="2 2" />;
    case "glasses-round":
      return (
        <>
          <circle cx="16" cy="24" r="6" fill="none" stroke={frame} strokeWidth="2" />
          <circle cx="32" cy="24" r="6" fill="none" stroke={frame} strokeWidth="2" />
          <line x1="22" y1="24" x2="26" y2="24" stroke={frame} strokeWidth="2" />
        </>
      );
    case "glasses-square":
      return (
        <>
          <rect x="10" y="18" width="12" height="10" fill="none" stroke={frame} strokeWidth="2" rx="2" />
          <rect x="26" y="18" width="12" height="10" fill="none" stroke={frame} strokeWidth="2" rx="2" />
          <line x1="22" y1="23" x2="26" y2="23" stroke={frame} strokeWidth="2" />
        </>
      );
    case "glasses-sunglasses":
      return (
        <>
          <ellipse cx="16" cy="24" rx="7" ry="5" fill="#1A1A1A" stroke={frame} strokeWidth="1.5" />
          <ellipse cx="32" cy="24" rx="7" ry="5" fill="#1A1A1A" stroke={frame} strokeWidth="1.5" />
          <line x1="23" y1="24" x2="25" y2="24" stroke={frame} strokeWidth="2" />
        </>
      );
    case "glasses-aviator":
      return (
        <>
          <path d="M9 20 Q16 18 23 22 L23 28 Q16 32 9 28 Z" fill="rgba(50,50,50,0.7)" stroke="#D4AF37" strokeWidth="1.5" />
          <path d="M25 22 Q32 18 39 20 L39 28 Q32 32 25 28 Z" fill="rgba(50,50,50,0.7)" stroke="#D4AF37" strokeWidth="1.5" />
        </>
      );
    case "glasses-nerd":
      return (
        <>
          <circle cx="16" cy="24" r="8" fill="none" stroke={frame} strokeWidth="2.5" />
          <circle cx="32" cy="24" r="8" fill="none" stroke={frame} strokeWidth="2.5" />
...
function AccessoryPreview({ accessoryId }: { accessoryId: string }) {
  switch (accessoryId) {
    ...
    case "acc-headphones":
      return (
        <>
          <path d="M10 24 Q10 10 24 8 Q38 10 38 24" fill="none" stroke="#333" strokeWidth="3" />
    ...
    case "acc-crown":
      return (
        <>
          <polygon points="24,4 14,16 18,16 12,12 16,20 20,14 24,20 28,14 32,20 36,12 30,16 34,16" fill="#FFD700" stroke="#DAA520" strokeWidth="1" />
    ...
    case "acc-halo":
      return <ellipse cx="24" cy="10" rx="14" ry="4" fill="none" stroke="#FFD700" strokeWidth="3" />;
    case "acc-earbuds":
      return (
        <>
          <ellipse cx="10" cy="28" rx="3" ry="3.5" fill="#F5F5F5" stroke="#E5E5E5" strokeWidth="1" />
          <ellipse cx="38" cy="28" rx="3" ry="3.5" fill="#F5F5F5" stroke="#E5E5E5" strokeWidth="1" />
          <path d="M10 32 Q10 40 16 44" stroke="#F5F5F5" strokeWidth="1.5" fill="none" />
          <path d="M38 32 Q38 40 32 44" stroke="#F5F5F5" strokeWidth="1.5" fill="none" />
        </>
      );
    default:
      return null;
...
function ClothingPreview({ clothingId }: { clothingId: string }) {
  switch (clothingId) {
    case "clothing-tshirt":
      return <path d="M10 44 Q10 38 24 36 Q38 38 38 44 L42 48 L6 48 Z" fill="#8B5CF6" />;
    case "clothing-hoodie":
      return (
        <>
          <path d="M10 44 Q10 38 24 36 Q38 38 38 44 L42 48 L6 48 Z" fill="#6B7280" />
          <line x1="18" y1="40" x2="18" y2="48" stroke="#9CA3AF" strokeWidth="1.5" />
          <line x1="30" y1="40" x2="30" y2="48" stroke="#9CA3AF" strokeWidth="1.5" />
        </>
      );
    case "clothing-jacket":
      return (
        <>
          ...
          <line x1="24" y1="36" x2="24" y2="48" stroke="#3B82F6" strokeWidth="2" />
        </>
      );
    case "clothing-sweater":
      return (
        <>
          <path d="M10 44 Q10 38 24 36 Q38 38 38 44 L42 48 L6 48 Z" fill="#DC2626" />
          <line x1="6" y1="44" x2="42" y2="44" stroke="#B91C1C" strokeWidth="1" />
          <line x1="6" y1="46" x2="42" y2="46" stroke="#B91C1C" strokeWidth="1" />
        </>
      );
    case "clothing-shirt":
      return (
        <>
          ...
          <path d="M20 36 L24 42 L28 36" fill="none" stroke="#E5E5E5" strokeWidth="1.5" />
        </>
      );
    case "clothing-tank":
